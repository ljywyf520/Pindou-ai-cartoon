...
} from '../conversion/featureSample.js'
...
function downscaleNearest(
  ...
  for (let row = 0; row < dstH; row++) {
    for (let col = 0; col < dstW; col++) {
      const sx = Math.min(
        srcW - 1,
        Math.max(0, Math.round(((col + 0.5) * srcW) / dstW - 0.5)),
      )
      const sy = Math.min(
        srcH - 1,
        Math.max(0, Math.round(((row + 0.5) * srcH) / dstH - 0.5)),
      )
...
function downscaleFeatureAware(
  pixels: Uint8ClampedArray,
  srcW: number,
  srcH: number,
  dstW: number,
 ...
 */
export function createPixelArtPrepPixels(
  pixels: Uint8ClampedArray,
  width: number,
  height: number,
  maxGrid: number,
  ...
  const simplified =
    needsDownscale && detailSensitive
