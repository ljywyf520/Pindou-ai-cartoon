...
import { computePrepTargetDimensions, suggestPrepColorCount, } from './prepParams.js';
...
function buildWeightedHistogram(pixels, width, height, edgeMap) {
    const hist = new Map();
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const i = (y * width + x) * 4;
...
function extractImportantColors(pixels, width, height, edgeMap, maxColors) {
    ...
    for (const item of ranked) {
        if (item.bucket.count >= minArea)
            pushColor(item.mean);
        if (picked.length >= maxColors)
            break;
    }
    for (const item of ranked) {
        if (item.bucket.edgeCount >= 2)
            pushColor(item.mean);
        if (picked.length >= maxColors)
            break;
    }
...
function sampleWeightedDominantRgb(pixels, imgWidth, edgeMap, x0, y0, x1, y1) {
...
function downscaleDominant(pixels, srcW, srcH, dstW, dstH, edgeMap = null) {
...
function downscaleEdgeMap(edgeMap, srcW, srcH, dstW, dstH) {
...
function downscaleDominantProgressive(pixels, srcW, srcH, dstW, dstH, edgeMap) {
    let cur = pixels;
    let w = srcW;
    let h = srcH;
    let curEdge = edgeMap;
...
function kMeansQuantizeWithSeeds(pixels, width, height, k, seeds) {
    ...
    while (centroids.length < k && centroids.length < samples.length) {
        ...
        for (let i = 0; i < samples.length; i++) {
            ...
            for (const c of centroids)
                min = Math.min(min, colorDistSq(samples[i], c));
            if (min > bestDist) {
                bestDist = min;
                bestIdx = i;
            }
...
function applyPalette(pixels, width, height, centroids, edgeMap, original) {
...
function smoothInteriorOnly(pixels, width, height, edgeMap, edgeThreshold = 0.3) {
    const out = new Uint8ClampedArray(pixels);
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const idx = y * width + x;
            const i = idx * 4;
            if (pixels[i + 3] < 128 || edgeMap[idx] >= edgeThreshold)
                continue;
            ...
            for (let dy = -1; dy <= 1; dy++) {
                for (let dx = -1; dx <= 1; dx++) {
                    ...
                    if (bucket)
                        ...
                        counts.set(key, { count: 1, r: pixels[ni], g: pixels[ni + 1], b: pixels[ni + 2] });
                }
            }
 ...
 */
export function createBeadPrepPixels(pixels, width, height, maxGrid) {
    ...
    const prepared = prepareSourcePixels(pixels, width, height, hints.isPhotoLike
        ? { brightness: 1, contrast: 12, saturation: 2 }
        : { brightness: 0, contrast: 8, saturation: 0 }, { denoise: hints.isPhotoLike, sharpen: hints.isPhotoLike });
    const edgeMap = computeEdgeMap(prepared, width, height);
    const downscaled = downscaleDominantProgressive(prepared, width, height, target.width, target.height, edgeMap);
    const naturalColors = countDistinctColors(downscaled.pixels, downscaled.width, downscaled.height);
    const colorBudget = suggestPrepColorCount(hints, naturalColors);
    const importantColors = extractImportantColors(downscaled.pixels, downscaled.width, downscaled.height, downscaled.edgeMap, colorBudget);
    const paletteSize = Math.max(importantColors.length, Math.min(colorBudget, naturalColors + 2));
    const centroids = kMeansQuantizeWithSeeds(downscaled.pixels, downscaled.width, downscaled.height, paletteSize, importantColors);
    let simplified = applyPalette(downscaled.pixels, downscaled.width, downscaled.height, centroids, downscaled.edgeMap, downscaled.pixels);
    simplified = smoothInteriorOnly(simplified, downscaled.width, downscaled.height, downscaled.edgeMap);
