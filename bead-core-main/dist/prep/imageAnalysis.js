/** 采样亮度方差，区分照片与卡通/插画 */
export function analyzeImageContent(pixels, width, height) {
 ...
 */
export function isDetailSensitiveCartoon(pixels, width, height, hints) {
    const content = hints ?? analyzeImageContent(pixels, width, height);
    if (content.isPhotoLike)
        return false;
    return countDistinctColors(pixels, width, height) <= 32;
}
/** 统计图像中近似 distinct 颜色数（默认 5 bit 分桶） */
export function countDistinctColors(pixels, width, height, bits = 5) {
    const shift = 8 - bits;
    const seen = new Set();
