 ...
 */
export function computePrepTargetDimensions(sourceWidth, sourceHeight, hints, maxGrid, distinctColors) {
    ...
    if (detailSensitive) {
        ...
        if (hints.variance > 200)
            targetShort = minEdge / 3.8;
        else if (hints.variance > 120)
            targetShort = minEdge / 3.4;
        else if (hints.variance > 80)
            targetShort = minEdge / 3.0;
        else
            targetShort = minEdge / 2.8;
...
export function suggestPrepColorCount(hints, naturalColors = 0) {
    let base;
...
/** 拼豆专用图二次出图：每个源像素对应一格 */
export function suggestGridWidthForPrepImage(imgWidth, _imgHeight, maxGrid) {
    return Math.max(40, Math.min(maxGrid, imgWidth));
}
export function suggestPrepMergeThreshold(colorCount, variance) {
    if (colorCount <= 20)
        return 0;
    if (colorCount > 28)
        return 2;
