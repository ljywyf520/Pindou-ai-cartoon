...
describe('convertImageToPattern', () => {
  ...
  it('snaps gray fringe pixels to palette colors in dominant mode', () => {
    ...
    const grid = convertImageToPattern(pixels, 16, 16, {
      gridWidth: 8,
      mode: 'dominant',
      palette,
      excludedPaletteIds: [],
    ...
    for (const row of grid) {
      for (const cell of row) {
        expect(['red', 'green', 'white', 'black']).toContain(cell.paletteId)
      }
    }
  })
  ...
  it('preserves thin outlines when downsampling in dominant mode', () => {
    const w = 80
    const h = 8
    const pixels = new Uint8ClampedArray(w * h * 4)
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        const i = (y * w + x) * 4
        const isLine = y === 3 || y === 4
        pixels[i] = isLine ? 0 : 255
        pixels[i + 1] = isLine ? 0 : 255
        pixels[i + 2] = isLine ? 0 : 255
        pixels[i + 3] = 255
      }
    }
    const grid = convertImageToPattern(pixels, w, h, {
      gridWidth: 20,
      mode: 'dominant',
      palette,
      excludedPaletteIds: [],
    })
    const blackCells = grid.flat().filter((cell) => cell.paletteId === 'black').length
    expect(blackCells).toBeGreaterThanOrEqual(2)
  })
})
